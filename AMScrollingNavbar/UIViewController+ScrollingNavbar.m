...
- (void)setLimit:(BOOL)limit { objc_setAssociatedObject(self, @selector(limit), [NSNumber numberWithBool:limit], OBJC_ASSOCIATION_RETAIN); }
- (BOOL)limit {	return [objc_getAssociatedObject(self, @selector(limit)) boolValue]; }
...
- (void)setUseCustomDeltaLimit:(BOOL)useCustomDeltaLimit {
    self.limit = useCustomDeltaLimit;
}
...
- (void)updateSizingWithDelta:(CGFloat)delta
    {
        ...
        frame.origin.y = frameNav.origin.y - 20;
